...
void main() {
  test('migrations are idempotent across repeated opens of the same file', () async {
    final path =
        '${Directory.systemTemp.createTempSync('homebox_localdb_').path}/test.sqlite3';
  ...
  test('openInMemory is immediately usable and isolated per instance', () {
    ...
    a.db.execute(
      "INSERT INTO sync_state (server_id, last_sync_revision, last_successful_sync_at) VALUES ('s', 5, '2026-01-01T00:00:00Z')",
    );
    expect(
      a.db
          .select('SELECT last_sync_revision FROM sync_state')
          .first['last_sync_revision'],
      5,
    );
