...
The Flutter client is in [client](client). It builds for Windows 11 and Android. Android declares `INTERNET` for pinned-TLS server access, requests `CAMERA` only when the user taps Camera in Files, and declares `USE_BIOMETRIC` to lock the app once at launch; it does not request broad device storage or gallery access. The biometric gate is used only when the device has an enrolled biometric method, and it keeps the UI hidden until the check succeeds. It is deliberately not re-checked on every background/foreground cycle: many in-app flows (the system file picker, the Android "Save As" picker, camera capture) briefly background the app as part of their own normal operation, and re-locking on each of those interrupted whatever was in progress as well as prompting far more often than intended. A camera photo stays in app-private temporary storage until it is immediately processed by the same encrypted upload flow as every other file. If Android recreates the activity while its camera is open, the client recovers the pending photo and uploads it once the signed-in vault is ready. The header (on every section, not only Files) duplicates the Sync page's own status — offline/up to date/syncing/paused/error — next to an icon-only vault-lock indicator (open/closed padlock, no "locked"/"unlocked" text pill; the fuller explanation is a tooltip and still lives in Settings), and, while a transfer is active, an arrow (↑ upload to the server, ↓ download from it) plus its percentage ahead of both. All three share their icon/label/percent logic with the Sync and Files pages' own copies rather than a separate implementation. The Files UI itself shows the same direction-and-percentage while a transfer is in progress, and each file's row shows its MIME type, decrypted size, and an "Updated" date/time (all derived from encrypted metadata the server never sees) — this reflects the most recent content change, including "Replace content…", not just the original upload; files uploaded before the size field existed simply omit it. On Android, pulling down on the Files list runs a sync pass against the server and re-lists the current folder, the same "Sync now" action as the Sync page's own trigger. Tapping a file name on Android opens the OS "Save As" picker (`ACTION_CREATE_DOCUMENT`, hinted to open in Downloads) instead of `file_selector`'s save dialog, which Android's platform implementation never supported — the file is decrypted to a private temp path first and deleted once the picker's chosen destination has the copy. Uploading a file (picked, dropped, or camera-captured) whose name already matches an existing file in the open folder prompts to overwrite, skip, or cancel before anything is sent; an overwrite goes through the same versioned "replace content" path as the Files menu action, rather than creating a duplicate node. The Files UI uses compact icon actions on Android. Device registration identifies Android clients as `ANDROID` and desktop clients as `WINDOWS`, so trusted-device and Family Vault records remain accurate. The current Files, Sync, and Settings UI is desktop-oriented: it uses the native Windows folder picker to select a local sync folder that mirrors the vault and uploads local changes back (see below). That local sync folder is Windows-only — the Sync page explains why on Android rather than offering a folder picker there: `file_selector`'s Android `getDirectoryPath` resolves to a raw filesystem path under scoped storage, which a picked-folder grant does not actually authorize `dart:io` to write into without the broad "All files access" permission, and HomeBox deliberately does not request that permission; Android users browse and download individual files from the Files page instead. The native Windows runner adds a notification-area icon using the app resource: closing hides the window, a left click or `Show HomeBox` restores it, and `Exit` is the explicit process shutdown. Credentials and file content both now travel over the pinned TLS connection to the server (see below).
