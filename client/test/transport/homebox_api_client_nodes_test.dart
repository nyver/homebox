...
void main() {
  test(
    'binary chunk upload and content download preserve non-UTF-8 bytes exactly',
    () async {
      ...
      expect(downloadUri?.queryParameters['versionId'], 'version with spaces');
    },
  );

  test(
    'node/version/sync JSON responses parse into the expected model objects',
    () async {
      final server = await startFixtureServer((request) async {
          ...
          request.response
            ...
            ..write(
              jsonEncode({
                ...
                'parentId': null,
                'nodeType': 'FILE',
                'metadataCiphertext': base64Encode(utf8.encode('meta')),
            ...
            path == '/api/v1/files/node-1/versions') {
          request.response
            ..statusCode = 200
            ..headers.contentType = ContentType.json
            ..write(
              jsonEncode([
                {
                  'id': 'version-1',
                  'blobId': 'blob-1',
                  'e2eeHeader': base64Encode([1, 2, 3]),
                  'wrappedFileKey': base64Encode([4, 5, 6]),
                  'keyScopeId': 'scope-1',
                  'keyVersion': 1,
                  'revision': 4,
                  'chunkCount': 1,
