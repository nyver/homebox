...
The Flutter client is in [client](client). It builds for Windows 11 and Android. Android declares `INTERNET` for pinned-TLS server access and requests `CAMERA` only when the user taps Camera in Files; it does not request broad device storage or gallery access. A camera photo stays in app-private temporary storage until it is immediately processed by the same encrypted upload flow as every other file. If Android recreates the activity while its camera is open, the client recovers the pending photo and uploads it once the signed-in vault is ready. Device registration identifies Android clients as `ANDROID` and desktop clients as `WINDOWS`, so trusted-device and Family Vault records remain accurate. The current Files, Sync, and Settings UI is desktop-oriented: it uses the native Windows folder picker to select a local sync folder that mirrors the vault and uploads local changes back (see below). The native Windows runner adds a notification-area icon using the app resource: closing hides the window, a left click or `Show HomeBox` restores it, and `Exit` is the explicit process shutdown. Credentials and file content both now travel over the pinned TLS connection to the server (see below).
