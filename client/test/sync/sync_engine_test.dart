...
PendingOperation _createOp(String nodeId, {String? parentId, DateTime? createdAt}) => PendingOperation(
      id: 'pending-$nodeId',
...
void main() {
  test('pushing a pending CREATE succeeds and the node lands in the local cache', () async {
    ...
    final engine = SyncEngine(serverConnection: serverConnection, localDatabase: LocalDatabase.openInMemory());
    ...
    expect(engine.pendingOperations.listReady(DateTime.now().add(const Duration(days: 1))), isEmpty);
  });
  ...
  test('an UPDATE queued right after a CREATE waits for the CREATE to finish instead of running out of order', () async {
    ...
    engine.pendingOperations.enqueue(PendingOperation(
      id: 'pending-update',
      operationId: 'op-update',
      type: PendingOperationType.updateNode,
      nodeId: 'node-1',
      payload: {'metadataCiphertext': base64Encode(utf8.encode('renamed')), 'metadataKeyVersion': 1},
      baseRevision: 1,
    ...
    final remainingReady = engine.pendingOperations.listReady(DateTime.now().add(const Duration(minutes: 10)));
    expect(remainingReady.map((o) => o.id), containsAll(['pending-node-1', 'pending-update']));
    final updateRow = remainingReady.firstWhere((o) => o.id == 'pending-update');
    expect(updateRow.retryCount, 0, reason: 'the update should never have been attempted');
    expect(engine.nodeCache.getById('node-1'), isNull, reason: 'the create never succeeded');
  });

  test('pulling changes discovers a node created by a different device', () async {
    final fakeServer = FakeNodeServer();
    final httpServer = await fakeServer.start();
    addTearDown(() => httpServer.close(force: true));
    final serverConnection = await _connectedAndSignedIn(httpServer);
  ...
  test('a transient failure (server error) schedules a backoff retry instead of failing permanently', () async {
    ...
    expect(engine.pendingOperations.listReady(DateTime.now()), isEmpty, reason: 'not ready yet, backoff window pending');
    final laterReady = engine.pendingOperations.listReady(DateTime.now().add(const Duration(minutes: 10)));
