...
Family Vault client transport (`client/lib/core/transport/homebox_api_client.dart`) now mirrors the server's read-only sharing contract: discovery of a recipient account's active device public keys, creation of a `READ` grant with opaque device envelopes, incoming/outgoing grant lists, and revoke. This layer does not unwrap or manufacture a folder key: the present client has only a personal vault key, and reusing it as a folder-sharing key would violate compartmentalization. Per-folder key creation, recipient envelope cryptography, and the Settings/Files UX remain the next client slice.
